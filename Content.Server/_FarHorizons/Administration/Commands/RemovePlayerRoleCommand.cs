...
public sealed class RemovePlayerRolesCommand : IConsoleCommand
{
    [Dependency] private readonly INullLinkPlayerManager _nullLinkPlayerManager = default!;
    [Dependency] private readonly IPlayerManager _playerManager = default!;

    public string Command => "removeplayerrole";
    public string Description => Loc.GetString("remove-player-role-command-description");
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        if (args.Length != 2)
        {
            shell.WriteLine(Loc.GetString("remove-player-role-wrong-args-error"));
            return;
        }

        if (!_playerManager.TryGetUserId(args[0], out var netUserId))
        {
            shell.WriteLine(Loc.GetString("remove-player-role-not-found-error"));
            return;
        }
