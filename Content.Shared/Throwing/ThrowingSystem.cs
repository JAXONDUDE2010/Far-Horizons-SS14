...
public sealed class ThrowingSystem : EntitySystem
{
        ...
        bool unanchor = false)
    {
        ...
        if (pushbackRatio == 0.0f ||
            physics.Mass == 0f ||
            !TryComp(pushbackTarget, out PhysicsComponent? userPhysics))
            return;
        ...
        RaiseLocalEvent(pushbackTarget, ref pushEv);
        const float massLimit = 5f;

        if (pushEv.Push)
            _physics.ApplyLinearImpulse(pushbackTarget, -impulseVector / physics.Mass * pushbackRatio * MathF.Min(massLimit, physics.Mass), body: userPhysics);
    }
}
